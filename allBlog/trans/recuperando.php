<?php include("log.php"); require ("conexao.php");
		...
		switch($_POST['mode']) {
			case "pass":
				...
				$sql=mysql_query("SELECT * FROM login WHERE usuario='$user' and email='$email'") or die (mysql_error());
				$row=mysql_num_rows($sql);
				if ($row>0){ while($linha=mysql_fetch_array($sql)){ // recebendo o valor das linhas
	...
	Sociace"; 
		...
		echo '<form name="pass" action="recuperar.php" method="POST">' .
			...
			case "user":
				...
				$sql=mysql_query("SELECT * FROM login WHERE senha='$senha' and email='$email'") or die (mysql_error());
				$row=mysql_num_rows($sql);
				if ($row>0){ while($linha=mysql_fetch_array($sql)){ // recebendo o valor das linhas
