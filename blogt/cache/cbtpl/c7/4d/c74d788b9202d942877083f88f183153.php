...
<feed xmlns="http://www.w3.org/2005/Atom"
  ...
  xml:lang="<?php echo context::global_filter($core->blog->settings->system->lang,0,0,0,0,0,'BlogLanguage'); ?>">
  ...
  <updated><?php echo context::global_filter(dt::iso8601($core->blog->upddt,$core->blog->settings->system->blog_timezone),0,0,0,0,0,'BlogUpdateDate'); ?></updated>
  <author>
    <name><?php echo context::global_filter($core->blog->settings->system->editor,1,0,0,0,0,'BlogEditor'); ?></name>
  </author>
...
<?php while ($_ctx->posts->fetch()) : ?>
  ...
  <entry>
    ...
    <published><?php echo context::global_filter($_ctx->posts->getISO8601Date(''),0,0,0,0,0,'EntryDate'); ?></published>
    <updated><?php echo context::global_filter($_ctx->posts->getISO8601Date('upddt'),0,0,0,0,0,'EntryDate'); ?></updated>
    ...
    <?php endif; ?>
    <?php
$_ctx->meta = $core->meta->getMetaRecordset($_ctx->posts->post_meta,'tag'); $_ctx->meta->sort('meta_id_lower','asc'); ?><?php while ($_ctx->meta->fetch()) : ?><dc:subject><?php echo context::global_filter($_ctx->meta->meta_id,0,0,0,0,0,'TagID'); ?></dc:subject><?php endwhile; $_ctx->meta = null; ?>
...
<?php foreach ($_ctx->attachments as $attach_i => $attach_f) : $GLOBALS['attach_i'] = $attach_i; $GLOBALS['attach_f'] = $attach_f;$_ctx->file_url = $attach_f->file_url; ?>
    ...
    <?php if($_ctx->posts->commentsActive()) : ?>
      ...
      <wfw:commentRss><?php echo context::global_filter($core->blog->url.$core->url->getURLFor("feed","atom"),0,0,0,0,0,'BlogFeedURL'); ?>/comments/<?php echo context::global_filter($_ctx->posts->post_id,0,0,0,0,0,'EntryID'); ?></wfw:commentRss>
    <?php endif; ?>
  </entry>
