<?php include("trans/log.php"); // call log function
		session_start(login);
		if ($_SESSION) { salvaLog($conexao, $_SESSION['login'] .": Visualizando", "allBlog/index.php"); include("trans/autentica.php"); }
		else { salvaLog("Visitante: Visualizando", "allBlog/index.php");require("trans/conexao.php"); /*conection with database*/ }
?>
...
<table class="headAll"><!-- Topinho com witdh 950 px height 50 px-->
  ...
  if ($_GET['n']){ // quando estiver chamando um post 
	$id=$_GET['n'];
	$s2=mysql_query("SELECT * FROM blog WHERE id='$id' ") or die (mysql_error()); $r2=mysql_num_rows($s2);
	if ($r2>0){ while($l2=mysql_fetch_array($s2)){
		...
		'<a onclick="javascript:location.href =\'index.php\';" style="cursor:pointer;">'.
		'Voltar</a></h6></td></tr>';
  }}
} else { // quando n�o chamar nada
  		
  $s2=mysql_query("SELECT * FROM blog WHERE visible='y' ORDER BY date DESC LIMIT 15 ") or die (mysql_error()); $r2=mysql_num_rows($s2);
  if ($r2>0){ while($l2=mysql_fetch_array($s2)){
